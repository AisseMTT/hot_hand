...
```{r echo=FALSE}
# populate_prev_shots(d_prev_mini)
# undebug(populate_prev_shots)



d_prev_mini2 <- d_prev_mini[1:18, ]

test1 <- plyr::ddply(d_prev_mini2, .variables = c("GAME_ID", "PERIOD"), .fun = get_time_between_shots) %>% select(GAME_ID, PERIOD, TOTAL_TIME_REMAINING, MINUTES_REMAINING, SECONDS_REMAINING, time_between_shots)
# plyr::ddply(d, .variables = c("PLAYER_ID", "GAME_ID"), .fun = get_streaks)
```
...
```{r}
d_prev_mini <- d_prev[1:1000,]

d_prev_mini <- d_prev_mini %>%
  mutate(is_hot = curr_hit_streak >= 3,
         is_cold = curr_miss_streak >= 3)
...
## Does streak predict shot distance?
```{r}
d_prev %>%
