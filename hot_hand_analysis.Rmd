...
run_test_df <- data.frame("PLAYER_NAME" = NA, "P_VALUE" = NA)
for (player in players_over_100_shots) {
  value <- runs.pvalue(subset(d_prev, as.character(PLAYER_NAME) == player)$SHOT_MADE_FLAG, pmethod="normal")
  run_test_df <- rbind(run_test_df, c("PLAYER_NAME" = player, "P_VALUE" = value))
}
